...
describe('VideoController (e2e)', () => {
  ...
  it('/videos/upload (POST) should upload a video', async () => {
    const filePath = '../video/ahatamatar-c921210b-ccc7-418b-b811-77afd6ff8cca.mp4';
  ...
  it('/videos/merge (POST) should merge multiple videos', async () => {
    ...
    expect(response.body).toHaveProperty('path');
    expect(response.body.duration).toBeGreaterThan(video1.duration + video2.duration);
  });
});